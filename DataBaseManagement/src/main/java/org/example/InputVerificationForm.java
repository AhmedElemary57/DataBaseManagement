...
public class InputVerificationForm extends JPanel {
    ...
    class MyVerifier extends InputVerifier implements ActionListener {
        ...
        protected void updateResults() {
            ...
            ratio = computeConsistency(replicas, readQuorum, writeQuorum);
    ...
    double computeConsistency(int replicas, int read, int write) {
        if (read+write > replicas) {
            return 1*100;
        }else {
            return 1-(choose(replicas-read,write)/choose(replicas,write));
        }
    }
